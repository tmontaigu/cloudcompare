...
Q_DECLARE_METATYPE(pdal::LasVLR)
...
/*
 * Used to filter Vlrs that are too much tied to
 * the point data and needs to be regenerated by pdal.
 *
 * Even if would store filtered vlrs, pdal would ignore them.
 */
bool vlrShouldBeSaved(const pdal::LasVLR &vlr) {
	if (vlr.userId() == "LASF_Projection")
	{
		return false;
	}
	else if (vlr.userId() == "LASF_Spec" && vlr.recordId() == 4)
	{
		// Extra Byte VLR, pdal will rewrite it if needed
		return false;
	}
	else {
		return true;
...
std::string toJsonString(const std::vector<pdal::LasVLR>& vlrs) {
	std::stringstream jsonStream;
	jsonStream << '[';
	for (size_t i = 0; i < vlrs.size(); ++i)
	{
		const pdal::LasVLR& vlr = vlrs[i];
		jsonStream << '{';
		jsonStream << "\"user_id\":" << std::quoted(vlr.userId()) << ',';
		jsonStream << "\"record_id\":" << vlr.recordId() << ",";
		jsonStream << "\"description\":" << std::quoted(vlr.description()) << ',';
//		jsonStream << "\"data\":" << std::quoted(vlr.data());
		jsonStream << "\"data\":" << '\"';
		for(size_t j = 0; j < vlr.dataLen(); ++j)
		{
			jsonStream << "x" << std::hex << int(vlr.data()[j]);
		}
		jsonStream << '\"';
		jsonStream << ((i == vlrs.size() - 1) ? "}\n" : "},\n");
	}
	jsonStream << ']';
...
CC_FILE_ERROR LASFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	try
	{
		...
		if (theCloud->hasMetaData(LAS_VLRS_META_DATA))
		{
			auto savedVlrs = theCloud->getMetaData(LAS_VLRS_META_DATA).value<std::vector<pdal::LasVLR>>();
			ccLog::Print("Num VLRS to save: %d", savedVlrs.size());
			writerOptions.add("vlrs", toJsonString(savedVlrs));
			theCloud->setMetaData(LAS_VLRS_META_DATA, QVariant::fromValue(savedVlrs));
		}
		...
		for (auto &chunk : chunks)
		{
			...
			if (loadedCloud)
			{
				if (loadedCloud->size())
				{
					...
					std::vector<pdal::LasVLR> vlrs;
					vlrs.reserve(lasHeader.vlrs().size());
					std::copy_if(
							lasHeader.vlrs().begin(),
							lasHeader.vlrs().end(),
							std::back_inserter(vlrs),
							vlrShouldBeSaved
					);
					loadedCloud->setMetaData(LAS_VLRS_META_DATA, QVariant::fromValue(vlrs));
